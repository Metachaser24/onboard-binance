...
// regex: [A-Za-z0-9]*appeal[A-Za-z0-9]*
template BinanceRegex(msg_bytes) {
	...
	for (var i = 0; i < msg_bytes; i++) {
		...
		is_substr0[i][4] <== is_substr0[i][3] + states[i+1][3] * states[i+2][4];
		is_substr0[i][5] <== is_substr0[i][4] + states[i+1][4] * states[i+2][5];
		is_substr0[i][6] <== is_substr0[i][5] + states[i+1][5] * states[i+2][6];
